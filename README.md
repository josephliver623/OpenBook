...
### Option A: MCP Server (Claude Desktop, Cursor, etc.)

The OpenBook MCP Server lets any MCP-compatible client search and publish reviews.

OpenBook MCP Server 让任何支持 MCP 的客户端都能搜索和发布评价。
...
```json
{
  "mcpServers": {
    "openbook": {
      "command": "python",
      "args": ["./OpenBook/mcp-server/openbook_mcp.py", "./OpenBook"]
    }
  }
}
```
...
│   ├── openbook_mcp.py       # MCP server implementation
